        ...
        const geminiRes = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`,
          {
            ...
            signal: geminiController.signal,
              ...
              contents: [{
                parts: [{
                  text: `以下のURLの商品ページにアクセスして、商品情報を詳細に抽出してください。
必ず純粋なJSONオブジェクトのみを返してください（説明文・前置き・マークダウン不要）。

URL: ${url}

## 抽出のポイント
- **商品名**: ブランド名を含む正式な商品名・モデル名を完全に取得する。略称は不可。
- **商品番号**: 品番・型番・アイテムナンバー・SKU・商品コード・管理番号など。URLのパス部分（例: /item/88138192）にも品番が含まれる場合があるので確認する。
- **商品詳細**: ページ内の商品説明文・素材・仕様・製造国・デザインコンセプトを300文字以上で詳しく記述する。
- **価格**: 税込価格を数字のみで記述（カンマ・円記号不要）。
- **カラー**: 商品の色名を記述。複数カラーある場合は選択中のカラーを優先。
- **ZOZOTOWNやEC系サイト**: 商品名がページ上部のh1タグやtitleタグにある。価格は税込価格。商品番号はURLのgoodsIdやdidパラメータ、またはページ内の「品番」「商品番号」「管理番号」等から取得。
- **ブランド公式サイト（N.HOOLYWOOD等）**: URLやページ内のSKU・品番をそのまま product_number に入れる。

以下のJSON形式のみを返してください:
{
  "name": "商品の正式名称（ブランド名＋モデル名、可能な限り詳しく）",
  "brand": "ブランド名",
  "product_number": "品番・型番・アイテムコード（URLやページから取得）",
  "price": "税込価格（数字のみ）",
  "currency": "JPY",
  "color": "色名",
  "category": "トップス/アウター／ジャケット/パンツ/その他（スーツ／ワンピース等）/バッグ/シューズ/アクセサリー／小物 のいずれか",
  "description": "商品詳細（ページ内の説明文・素材・仕様・デザイン特徴・製造国を300文字以上）",
  "image_urls": ["画像URL1", "画像URL2"],
  "available_colors": [{"name": "色名", "image_url": "画像URL"}],
  "available_sizes": ["S", "M", "L"]
}

image_urlsには以下の画像URL候補から商品の実物画像を最大3枚選んで入れてください（ロゴ・バナー・アイコンは除く）:
